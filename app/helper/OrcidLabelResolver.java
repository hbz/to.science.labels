...
public class OrcidLabelResolver {
    ...
    public static String lookup(String uri) {
        ...
        try (InputStream in = RdfUtils.urlToInputStream(new URL(uri), "application/json")) {
            ...
            String label = hit.at("/orcid-profile/orcid-bio/personal-details/family-name/value").asText() + ", "
                    + hit.at("/orcid-profile/orcid-bio/personal-details/given-names/value").asText();
            return label;
        } catch (Exception e) {
            play.Logger.warn("", e);
        }
